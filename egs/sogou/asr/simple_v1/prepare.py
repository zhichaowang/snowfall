...
from lhotse.recipes import prepare_sogou, prepare_musan
...
def main():
    corpus_dir = locate_corpus(
        (Path('/search/speech/wangzhichao/database/train_sogou_500h'),
...
#         Path('/nfsdata/database/musan'),
#         Path('/root/fangjun/data/musan')),
#        msg='Please specify the directory to the MUSAN dataset')

    output_dir = Path('exp/data')
    print('sogou manifest preparation:')
    sogou_manifests = prepare_sogou(
...
#    )
    ...
    with get_executor() as ex:  # Initialize the executor only once.
        for partition, manifests in sogou_manifests.items():
...
#                cut_set = cut_set + cut_set.perturb_speed(0.9) + cut_set.perturb_speed(1.1)
            ...
            sogou_manifests[partition]['cuts'] = cut_set
