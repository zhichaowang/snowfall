...
from snowfall.common import load_checkpoint, save_checkpoint, load_pretrained_model
from snowfall.common import save_training_info
from snowfall.common import setup_logger
from snowfall.data.sogou import SogouAsrDataModule
...
def get_objf(batch: Dict,
    ...
    if is_training:
        def maybe_log_gradients(tag: str):
            if tb_writer is not None and global_batch_idx_train is not None and global_batch_idx_train % 1000 == 0:
        ...
        if is_update:
            ...
            if tb_writer is not None and (global_batch_idx_train // accum_grad) % 1000 == 0:
...
def train_one_epoch(dataloader: torch.utils.data.DataLoader,
    ...
    for batch_idx, batch in enumerate(dataloader):
        ...
        if batch_idx > 0 and batch_idx % 1600 == 0:
...
def run(rank, world_size, args):
    ...
    exp_dir = Path('exp-' + model_type + '-noam-mmi-att-sogou500h-syllable-prune')
    if rank == 0:
        setup_logger('{}/log/log-train'.format(exp_dir), log_level='info')
    ...
    lang_dir = Path('data/lang_0528_syllable_nosp')
    ...
    train_dl = sogou.train_dataloaders()
    valid_dl = sogou.valid_dataloaders()
    ...
    if init_model > 0:
        model_path = os.path.join(exp_dir, 'epoch-{}.pt'.format(init_model))
        ckpt = load_pretrained_model(filename=model_path, model=model)
