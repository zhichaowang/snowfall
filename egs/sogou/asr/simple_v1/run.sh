...
if [ $stage -le 4 ]; then
  echo "LM training..."
  if $train_lm; then
    local2/sogou_train_lms.sh data/local/train/text data/local/dict_0528/lexicon.txt data/local/lm
...
#  python3 -m torch.distributed.launch --nproc_per_node=$ngpus ./mmi_bigram_train.py --world_size $ngpus
  ...
  CUDA_VISIBLE_DEVICES=0 python3 ./mmi_att_transformer_train.py --world-size=1 --amp=False --use-ali-model=False --max-duration=300 --num-epochs=15 --master-port=12360 --concatenate-cuts=False --bucketing-sampler=True --num-buckets=500
  CUDA_VISIBLE_DEVICES=4,5,6,7 python3 ./mmi_att_transformer_train_new.py --world-size=4 --amp=False --use-ali-model=False --max-duration=300 --num-epochs=25 --master-port=12360 --warm-step=25000
fi
...
if [ $stage -le 7 ]; then
  ...
  CUDA_VISIBLE_DEVICES=7 ./mmi_att_transformer_decode_syllable.py --epoch=6 --avg=1 --use-lm-rescoring=False --num-path=100 --max-duration=10 --output-beam-size=7
fi
