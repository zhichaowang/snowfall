...
from snowfall.training.mmi_graph import create_bigram_phone_lm, create_unigram_phone_lm
...
def main():
    ...
    exp_dir = Path('exp-' + model_type + '-noam-mmi-att-sogou500h-4gpu-dur300-K2Conformer-unigram-char-att0.5')
    setup_logger('{}/log/log-decode'.format(exp_dir), log_level='debug')

    logging.info(f'output_beam_size: {output_beam_size}')

    # load L, G, symbol_table
    lang_dir = Path('data/lang_0528_char_nosp')
...
#    print_transition_probabilities(P, phone_symbol_table, phone_ids, filename='P_scores.txt')
    ...
    if use_lm_rescoring:
        ...
        first_word_disambig_id = find_first_disambig_symbol(symbol_table)
        if not os.path.exists(lang_dir / 'G_5_gram.pt'):
            logging.debug('Loading G_5_gram.fst.txt')
            with open(lang_dir / 'G_5_gram.fst.txt') as f:
                G = k2.Fsa.from_openfst(f.read(), acceptor=False)
                ...
                G = k2.arc_sort(G)
                torch.save(G.as_dict(), lang_dir / 'G_5_gram.pt')
        else:
            logging.debug('Loading pre-compiled G_5_gram.pt')
            d = torch.load(lang_dir / 'G_5_gram.pt')
...
#            G = G.to(device)
    ...
    # load dataset
    sogou = SogouAsrDataModule(args)
    test_sets = ['test8000', 'testIOS', 'not_on_screen', 'testDuiHua']
    #  test_sets = ['test-other']
    for test_set, test_dl in zip(test_sets, sogou.test_dataloaders()):
        ...
        logging.info(
            f'[{test_set}] %CER {errors2["total"] / total_chars:.2%} '
            f'[{errors2["total"]} / {total_chars}, {errors2["ins"]} ins, {errors2["del"]} del, {errors2["sub"]} sub ]'
        )

#        errs_filename = exp_dir / f'errs-{test_set}.txt'
#        print(f"%WER = {errors["total"] / total_words:.2%}", file=f)
#        print(f"{errors["total"]} / {total_words}, {errors["ins"]} ins, {errors["del"]} del, {errors["sub"]} sub")
