...
if [ $stage -le 3 ]; then
  ...
  local/prepare_lang.sh \
    ...
    data/local/dict_0528_syllable \
    "<UNK>" \
    data/local/lang_tmp_syllable_nosp \
    data/lang_0528_syllable_nosp
...
if [ $stage -le 4 ]; then
  echo "LM training..."
  if $train_lm; then
    local2/sogou_train_lms.sh data/local/train/text data/local/dict_0528_syllable/lexicon.txt data/local/lm_syllable
    gunzip -c data/local/lm_syllable/3gram-mincount/lm_unpruned.gz >data/local/lm_syllable/lm_tgmed.arpa
  fi

  # Build G
  python3 -m kaldilm \
    --read-symbol-table="data/lang_0528_syllable_nosp/words.txt" \
    --disambig-symbol='#0' \
    --max-order=1 \
    data/local/lm_syllable/lm_tgmed.arpa >data/lang_0528_syllable_nosp/G_uni.fst.txt

  python3 -m kaldilm \
    --read-symbol-table="data/lang_0528_syllable_nosp/words.txt" \
    --disambig-symbol='#0' \
    --max-order=3 \
    data/local/lm_syllable/lm_tgmed.arpa >data/lang_0528_syllable_nosp/G.fst.txt
